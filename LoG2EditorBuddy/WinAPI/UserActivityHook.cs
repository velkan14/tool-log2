...
namespace gma.System.Windows
{
    ...
    public class UserActivityHook
    {
        ...
        [DllImport("user32.dll")]
        static extern int SetFocus(IntPtr point);
        [DllImport("user32.dll")]
        ...
        ///// <summary>
        ///// Occurs when the user releases mouse button
        ///// </summary>
        //public event MouseEventHandler On;
        /// <summary>
        ...
        private string GetApplicationName(IntPtr windowHandle)
        {
        ...
        public string sendKeystroke(ushort k, IntPtr hWin)
        {
            ...
            IntPtr result3 = SendMessage(hWin, WM_KEYDOWN, ((IntPtr)k), (IntPtr)0);
        ...
        public void SendSaveCommand(IntPtr hWin)
        {
            ...
            sim.Keyboard.KeyUp(WindowsInput.Native.VirtualKeyCode.CONTROL); //dont forget the key up
        ...
        {
            //InputSimulator sim = new InputSimulator();
            Debug.WriteLine("Sending reload command");

            //atach input threads because it may sometimes not work for OS's above xp (bugged, need to reload manually)
            //AttachThreadInput(
            //    GetWindowThreadProcessId(GetForegroundWindow(), IntPtr.Zero),
            //    GetWindowThreadProcessId(log2.MainWindowHandle, IntPtr.Zero),
            //    true);
            if (APIClass.CurrentMap == null)
                return;

            SetForegroundWindow(log2.MainWindowHandle);
            SetFocus(log2.MainWindowHandle);

            //restore if minimized
            if(IsIconic(log2.MainWindowHandle))
                ShowWindow(log2.MainWindowHandle, SW_RESTORE);
            else
                ShowWindow(log2.MainWindowHandle, SW_SHOWNORMAL);
            //SendKeys.SendWait("^R");
        ...
        public void Start(bool InstallMouseHook, bool InstallKeyboardHook)
        {
            ...
            if (hMouseHook == 0 && InstallMouseHook)
            {
                ...
                if (log2instances.Length == 0)
                    //Debug.WriteLine("No instances of Legend of Grimrock 2 Editor found.");
                    Logger.AppendText(StringResources.StartGrimrockString);
                else
                    foreach (Process n in log2instances)
                    {
                        //Debug.WriteLine("Found process: " + n.ProcessName + " with PID: " + n.Id);
                        //Logger.AppendText("Found process: " + n.ProcessName + " with PID: " + n.Id);
                ...
                if (log2 != null)
                {
                    ...
                    else if (hMouseHook != 0)
                        Logger.AppendText("Hook attached.");
                }
            }
